...
for (run in runs)
{
  ...
  peptideCount <- data %>% dplyr::select(Sequence, Modifications) %>% distinct %>% nrow
  ...
  proteinCount <- data %>% dplyr::select(Protein) %>% distinct %>% nrow
  ...
  data <- MSnSet(
    exprs = data %>% dplyr::select(one_of(sampleColumns)) %>% as.matrix,
    fData = data %>% dplyr::select(Sequence, Modifications, Protein),
...
print(
  xtable(
    metadata %>% dplyr::select(Run, Tag, Group) %>% spread(Run, Tag),
...
countProteins <- function(data) data %>% fData %>% dplyr::select(Protein) %>% distinct %>% nrow
...
counts <- counts %>% dplyr::select(-label)
...
for (missingValueMethod in c("IncludeMissingValues", "ExcludeMissingValues", "ImputeMin", "ImputeKNN"))
{
  ...
  for (run in runs)
  {
    ...
    controlIntensities <- data %>%
      exprs %>%
      as.data.frame %>%
      dplyr::select(one_of(controlGroup)) %>%
      unlist %>%
      as.numeric
...
for (analysis in names(peptideDataNormalized))
{
  ...
  for (run in runs)
  {
    ...
    fData(data) <- fData(data) %>% dplyr::select(Protein)

    proteinData[[analysis]][[run]] <- data
    pData(proteinData[[analysis]][[run]])<-pData(peptideDataForSummarization[[run]]) ##Added by ANH
    }
}
```
...
getReplicateCounts <- function(data)
{
  ...
  for (run in runs)
  {
    ...
    intensities <- intensities %>%
      mutate(N = N + apply(dplyr::select(., one_of(samples)), 1, isRepresented))
  }
...
replicateCountTableData <- inner_join(
  getReplicateCounts(combinedProteinData[["ImputeKNN_ScaleNormalization"]]) %>%
    dplyr::rename(`Including missing values` = Count),
  getReplicateCounts(combinedProteinData[["ExcludeMissingValues_ScaleNormalization"]]) %>%
    dplyr::rename(`Excluding missing values` = Count),
  by = "N"
)
  ...
  proteinInfo)
{
  ...
  contrastFittedIntensities <- dplyr::select(fittedIntensities, one_of(reformattedContrastGroups))
  ...
  results <- results %>%
    mutate(N = n) %>%
    bind_cols(fittedIntensities %>% dplyr::select(one_of(rev(reformattedContrastGroups), controlGroup))) %>%
    bind_cols(intensities) %>%
    left_join(proteinInfo, by = "Protein")

  results <- bind_cols(
    results %>% dplyr::select(Protein, Gene, Description, N),
    results %>% dplyr::select(-Protein, -Gene, -Description, -N)
  )
...
results <- inner_join(
  results1 %>% dplyr::select(Protein, B1 = B),
  results2 %>% dplyr::select(Protein, B2 = B),
  by = "Protein"
  ) %>%
  filter(complete.cases(.)) %>%
                                 ...
                                 controlLogFoldChangeThreshold = 1.0)
{
  resultsTable <- results %>%
    dplyr::select(Protein, Gene, N, logFC, AveExpr, adj.P.Val, B, logFCcontrol)
